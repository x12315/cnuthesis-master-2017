...
本章围绕最大相关熵卡尔曼滤波（Maximum Correntropy Kalman Filter，MCKF）及其带宽自适应策略展开。相较于以最小均方误差为目标的扩展卡尔曼滤波（Extended Kalman Filter，EKF），最大相关熵卡尔曼滤波（MCKF）的核心改进在于将相关熵原则引入滤波代价函数，用非线性核函数对残差进行重加权，从而降低异常值和复杂噪声对状态估计的影响\cite{izanloo2016kalman,CHEN201770}。相关熵可以理解为在希尔伯特空间中度量两个随机变量相似性的指标，借助 Mercer 核将误差映射到高维特征空间后，再通过样本平均近似其期望值，因此能够对误差分布的高阶信息进行建模。本文选用高斯核作为相关函数，一方面是因为高斯核光滑、可微且便于迭代求解，另一方面也是由于其具备较强的通用逼近能力，在理论上可以刻画多种形式的误差代价函数。基于这一思想，MCKF 不再仅依赖二次型误差来度量偏差，而是通过对大残差赋予更低权重，提升了复杂噪声环境下的鲁棒性\cite{xie2025adaptive}。
...
\begin{equation}
    \hat{P}_{s_k} = \frac{1}{N} \sum_{i=k-N+1}^{k} \bm{s}_i \bm{s}_i^{\mathrm{T}},
